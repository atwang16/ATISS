...
def main(argv):
    ...
    parser.add_argument("path_to_3d_front_dataset_directory", help="Path to the 3D-FRONT dataset")
    parser.add_argument("path_to_3d_future_dataset_directory", help="Path to the 3D-FUTURE dataset")
    parser.add_argument("path_to_model_info", help="Path to the 3D-FUTURE model_info.json file")
    ...
    parser.add_argument(
        "--path_to_invalid_bbox_jids", default="../config/black_list.txt", help="Path to objects that are blacklisted"
    )
    parser.add_argument(
        "--annotation_file",
    ...
    parser.add_argument(
        "--up_vector", type=lambda x: tuple(map(float, x.split(","))), default="0,0,-1", help="Up vector of the scene"
    )
    parser.add_argument(
        "--background",
    ...
    for (i, es), ss in tqdm(zip(enumerate(encoded_dataset), dataset)):
        ...
        with DirLock(room_directory + ".lock") as lock:
            ...
            try:
                ...
                print(f"Scene type: {ss.uid}")
                traceback.print_exc()
                print("Skipping final render...")
